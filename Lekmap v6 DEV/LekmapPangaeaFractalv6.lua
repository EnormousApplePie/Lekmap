...
include("Lekmap_MapGenerator");
include("Lekmap_FractalWorld");
include("Lekmap_FeatureGenerator");
include("Lekmap_TerrainGenerator");
include("IslandMaker");
...
function GetMapScriptInfo()
	...
	return {
		...
		CustomOptions = {
			----------------------------------------------------------------
			-- OPTION 1: [Terrain] World Age
			----------------------------------------------------------------
			{
				Name = "[Terrain] World Age",
				Values = {
			...
			----------------------------------------------------------------
			-- OPTION 2: [Terrain] Temperature
			----------------------------------------------------------------
			{
				Name = "[Terrain] Temperature",
				Values = {
			...
			----------------------------------------------------------------
			-- OPTION 3: [Terrain] Rainfall
			----------------------------------------------------------------
			{
				Name = "[Terrain] Rainfall",
				Values = {
			...
			----------------------------------------------------------------
			-- OPTION 6: [Spawns] Start Distance
			----------------------------------------------------------------
			{
				Name = "[Spawns] Start Distance",
				Values = {
					"Close",
					"Normal",
			...
			----------------------------------------------------------------
			-- OPTION 7: [Resources] Natural Wonders
			----------------------------------------------------------------
			{
				Name = "[Resources] Natural Wonders",
				Values = {
					"0",
					"1",
					"2",
			...
			----------------------------------------------------------------
			-- OPTION 8: [Terrain] Grass Moisture
			----------------------------------------------------------------
			{
				Name = "[Terrain] Grass Moisture",
				Values = {
					"Wet",
					"Normal",
					"Dry",
			...
			----------------------------------------------------------------
			-- OPTION 11: [Map Size] Land Size X
			----------------------------------------------------------------
			{
				Name = "[Map Size] Land Size X",
				Values = {
					"30",
					"32",
					"34",
			...
			----------------------------------------------------------------
			-- OPTION 12: [Map Size] Land Size Y
			----------------------------------------------------------------
			{
				Name = "[Map Size] Land Size Y",
				Values = {
					"20",
					"22",
					"24",
			...
			----------------------------------------------------------------
			-- OPTION 13: [Resources] Density
			----------------------------------------------------------------
			{
				Name = "[Resources] Density",
				Values = {
					"1 -- Nearly Nothing",
					"2",
					"3",
			...
			----------------------------------------------------------------
			-- OPTION 14: [Resources] Balanced Regionals
			----------------------------------------------------------------
			{
				Name = "[Resources] Balanced Regionals",
				Values = {
					"Yes",
					"No",
				},
			...
			----------------------------------------------------------------
			-- OPTION 15: [Map Size] Islands
			----------------------------------------------------------------
			{
				Name = "[Map Size] Islands",
				Values = {
					"No Islands",
					"1",
					"2",
			...
			----------------------------------------------------------------
			-- OPTION 16: [Spawns] Coastal Spawns
			----------------------------------------------------------------
			{
				Name = "[Spawns] Coastal Spawns",
				Values = {
					"Coastal Civs Only",
					"Random",
					"Random+ (~2 coastals)",
			...
			----------------------------------------------------------------
			-- OPTION 17: [Spawns] Coastal Luxuries
			----------------------------------------------------------------
			{
			...
			----------------------------------------------------------------
			-- OPTION 18: [Spawns] Inland Sea Spawns
			----------------------------------------------------------------
			{
				Name = "[Spawns] Inland Sea Spawns",
				Values = {
					"Allowed",
					"Not allowed",
				},
			...
			----------------------------------------------------------------
			-- OPTION 19: [Features] Fjord Distance
			----------------------------------------------------------------
			{
				Name = "[Features] Fjord Distance",
				Values = {
					"No fjords",
					"20 tiles",
					"15 tiles",
			...
			----------------------------------------------------------------
			-- OPTION 20: [Features] Fjord Length
			----------------------------------------------------------------
			{
				Name = "[Features] Fjord Length",
				Values = {
					"2 tiles",
					"3 tiles",
					"4 tiles -- Default",
...
function PangaeaFractalWorld.Create(fracXExp, fracYExp)
	...
	local data = {
		...
		num_plots_x = gridWidth,
		num_plots_y = gridHeight,
		plot_types = table.fill(PlotTypes.PLOT_OCEAN, gridWidth * gridHeight),
	};
...
function PangaeaFractalWorld:GeneratePlotTypes(args)
	...
	while allcomplete == false do
		...
		local hills_ridge_flags = self.fractal_flags;
		local peaks_ridge_flags = self.fractal_flags;
		...
		while bMapOK == false do
			...
			while done == false do
				...
				self.continents_frac = nil;
				self:InitFractal{continent_grain = grain_dice, rift_grain = rift_dice};
				iWaterThreshold = self.continents_frac:GetHeight(water_percent);
			...
			self.hills_frac:BuildRidges(numPlates, hills_ridge_flags, 1, 2);
			self.mountains_frac:BuildRidges((numPlates * 2) / 3, peaks_ridge_flags, 6, 1);
			-- Get height values
			local iHillsBottom1 = self.hills_frac:GetHeight(hillsBottom1);
			local iHillsTop1 = self.hills_frac:GetHeight(hillsTop1);
			local iHillsBottom2 = self.hills_frac:GetHeight(hillsBottom2);
			...
			local iMountain99 = self.mountains_frac:GetHeight(99);
			local iMountain97 = self.mountains_frac:GetHeight(97);
			local iMountain95 = self.mountains_frac:GetHeight(95);
		...
		if fjord_distance_setting ~= 1 then
			...
			local fjord_length_setting = Map.GetCustomOption(20);
			local fjord_l;
			...
			if self.plot_types[plotIndex] == PlotTypes.PLOT_OCEAN then
				...
				if islLandInRing ~= 0 then
					...
					if plotTypesTwo[landPlot] == PlotTypes.PLOT_LAND then
						...
						--self.plot_types[startingPlot] = PlotTypes.PLOT_LAND
						...
						if islLandInRing > minIslandSize and islLandInRing < maxIslandSize then
							...
							if (locationRnd > hill_thresh) then
								...
								self.plot_types[startingPlot] = PlotTypes.PLOT_HILLS;
							end
										...
										elseif wrapY == false and (nextY < 0 or nextY >= iH) then
											...
											if islRand > islThresh then
												...
												if islHill <= inner_hill_thresh then
													self.plot_types[plotIndex] = PlotTypes.PLOT_LAND
												else
													self.plot_types[plotIndex] = PlotTypes.PLOT_HILLS
												end
											end
...
function GenerateTerrain()
	...
	local args = {
			temperature = temp,
			desert_percent = DesertPercent,
			grass_moist = grassMoist,
			};
...
function StartPlotSystem()
	...
	if coastalSetting == 1 then      -- "Coastal Civs Only"
		noCoastInland = true;
	elseif coastalSetting == 3 then  -- "Random+ (~2 coastals)"
		noCoastInland   = true;
	...
	------------------------------------------------------------------------------
	print("Lekmap: Initializing impact layers.");
	Lekmap_Impact.Initialize();

	------------------------------------------------------------------------------
	-- 2. Generate regions.
	------------------------------------------------------------------------------
	local numCivs, numCityStates, playerList = Lekmap_Utilities.GetPlayerAndTeamInfo();
	print(string.format("Lekmap: %d civs, %d city-states detected.", numCivs, numCityStates));

	print("Lekmap: Generating regions.");
	...
	------------------------------------------------------------------------------
	-- 3. Choose player spawn locations.
	------------------------------------------------------------------------------
	local startDistance = Map.GetCustomOption(6);

	print("Lekmap: Choosing start locations.");
	Lekmap_Spawns.ChooseLocations({
	...
	------------------------------------------------------------------------------
	-- 4. Initialize resource system (enum lookups, defs, plot cache).
	--    Must happen before luxury assignment.
	------------------------------------------------------------------------------
	...
	------------------------------------------------------------------------------
	-- 5. Assign luxury roles (regional, CS, random).
	...
	------------------------------------------------------------------------------
	-- 6. Place City States.
	--    Placed before resources so that resource placement respects CS impacts.
	------------------------------------------------------------------------------
	print("Lekmap: Placing City States.");
	Lekmap_CityStates.PlaceAll({
		numCityStates = numCityStates,
		numCivs       = numCivs,
	});

	------------------------------------------------------------------------------
	-- 7. Place Natural Wonders.
	------------------------------------------------------------------------------
	local wonderOption = Map.GetCustomOption(7);
	local wonderAmount;
	...
	--    Luxury assignment already done in step 5.
	------------------------------------------------------------------------------
